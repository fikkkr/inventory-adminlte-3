...
@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title mb-0">Form Edit Barang Masuk</h3>
        </div>
        ...
        <form action="{{ route('barangMasuk.update', $barangMasuk->id) }}" method="POST">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <select name="id_barang" id="id_barang" class="form-control @error('id_barang') is-invalid @enderror" required>
                        ...
                        @foreach ($barangs as $item)
                            <option value="{{ $item->id }}" {{ old('id_barang', $barangMasuk->id_barang) == $item->id ? 'selected' : '' }}>
                                {{ $item->nama_barang }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="jumlah_masuk" id="jumlah_masuk" class="form-control @error('jumlah_masuk') is-invalid @enderror" value="{{ old('jumlah_masuk', $barangMasuk->jumlah_masuk) }}" required>
                    @error('jumlah_masuk')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
